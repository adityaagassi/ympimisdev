									...
									<div id="budgetket" style="display: none">
										...
										<div class="col-xs-12">
											...
											<table class="table" style="border:none">
												...
												<tr>
													<td style="border:none;text-align: left;padding-left: 0;width: 8%">Sisa Budget (3 Minggu Kedepan)</td>
													<td style="border:none;text-align: left;padding-left: 0;width: 2%">:</td>
													<td style="border:none;text-align: left;padding-left: 0;width: 90%"><label id="sisa_budget" name="sisa_budget"></label></td>
												</tr>
											</table>
						...
						<div class="tab-pane" id="tab_3">
							<div class="row">
								<div class="col-md-12" style="margin-bottom : 5px">
									...
									<div class="col-xs-2" style="padding:5px;">
										<input type="text" class="form-control" id="item_spec1" name="item_spec1" placeholder="Specification" required="">
									</div>
									<div class="col-xs-1" style="padding:5px;">
...
<script>
		...
		if (elem.value == "kosong") {
			...
			$.ajax({
	            ...
	            success: function(data) {
	                ...
	                $('#uom'+no).val(obj.uom).attr('readonly', true);
	...
	function pilihBudget(elem)
	{
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                if (obj.bulan == "04" || obj.bulan == "05" || obj.bulan == "06") {
                	quarter = "Q1";

                	if (obj.bulan == "04") {
	                	$('#budget'+quarter).text("$"+obj.apr);
	                	$('#sisa_budget').text("$"+obj.apr);
	                }

	                else if (obj.bulan == "05") {
	                	$('#budget'+quarter).text("$"+obj.may);
	                	$('#sisa_budget').text("$"+obj.may);
	                }

	                else if (obj.bulan == "06") {
	                	$('#budget'+quarter).text("$"+obj.jun);
	                	$('#sisa_budget').text("$"+obj.jun);
	                }
                }

                else if (obj.bulan == "07" || obj.bulan == "08" || obj.bulan == "09"){
   					quarter = "Q2";

   					if (obj.bulan == "07") {
	                	$('#budget'+quarter).text("$"+obj.jul);
	                	$('#sisa_budget').text("$"+obj.jul);
	                }

	                else if (obj.bulan == "08") {
	                	$('#budget'+quarter).text("$"+obj.aug);
	                	$('#sisa_budget').text("$"+obj.aug);
	                }

	                else if (obj.bulan == "09") {
	                	$('#budget'+quarter).text("$"+obj.sep);
	                	$('#sisa_budget').text("$"+obj.sep);
	                }
   					             	
                }


                else if (obj.bulan == "10" || obj.bulan == "11" || obj.bulan == "12"){
   					quarter = "Q3";

   					if (obj.bulan == "10") {
                		$('#budget'+quarter).text("$"+obj.oct);
                		$('#sisa_budget').text("$"+obj.oct);
	                }

	                else if (obj.bulan == "11") {
	                	$('#budget'+quarter).text("$"+obj.nov);
	                	$('#sisa_budget').text("$"+obj.nov);
	                }

	                else if (obj.bulan == "12") {
	                	$('#budget'+quarter).text("$"+obj.dev);
	                	$('#sisa_budget').text("$"+obj.dev);
	                }   					             	
                }


                else if (obj.bulan == "01" || obj.bulan == "02" || obj.bulan == "03"){
                	quarter = "Q4";

	                if (obj.bulan == "01") {
	                	$('#budget'+quarter).text("$"+obj.jan);
	                	$('#sisa_budget').text("$"+obj.jan);
	                }

	                else if (obj.bulan == "02") {
	                	$('#budget'+quarter).text("$"+obj.feb);
	                	$('#sisa_budget').text("$"+obj.feb);
	                }

	                else if (obj.bulan == "03") {
	                	$('#budget'+quarter).text("$"+obj.mar);
	                	$('#sisa_budget').text("$"+obj.mar);
	                }
                }
	...
	function getTotal(id) {
    	...
    	if (!isNaN(hasil)) {
    	    ...
    	    for (var i = 1; i < no; i++) {
    	    	...
    	    	konversiToUSD(total_usd, total_yen, total_id, curr,'USD');
    ...
    function konversiToUSD(amount_usd, amount_yen, amount_id, from, to){
    	if (from != "USD") {
	    	$.ajax({
	           ...
	           success : function(data){
	            	...
	            	if (from == "JPN") {
			            hasil_konversi_yen = (amount_yen / fromrate) * torate;
			    		// console.log(hasil_konversi_yen);
	            	}

	            	if (from == "ID") {
			            hasil_konversi_id = (amount_id / fromrate) * torate;
				    	// console.log(hasil_konversi_id);
	            	}
			    	...
			    	//Get Budget + Total Pembelian
			    	budget = $('#budget'+quarter).text();
			    	total = document.getElementById('total_keseluruhan').value;
	                $('#totalPembelian'+quarter).text("$"+total); ;

	                //Get Sisa Budget

	                var sisa = parseFloat(budget.substr(1)) - parseFloat(total);
	                $('#sisaBudget'+quarter).text("$"+sisa.toFixed(2));
