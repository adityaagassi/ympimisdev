...
<section class="content">
	...
	<div class="row">
		<div class="col-xs-5">
			<div class="box">
				<div class="box-body">
					<span style="font-size: 20px; font-weight: bold;text">SCAN ITEM:</span>
					<div class="input-group col-md-12">
						<div class="input-group-addon" id="icon-serial" style="font-weight: bold; font-size: 2vw; border-color: black;">
							<i class="glyphicon glyphicon-credit-card"></i>
						</div>
						<input type="text" style="text-align: center; border-color: black; font-size: 2vw; height: 50px" class="form-control" id="tag_product" name="tag_product" placeholder="Scan Tag Here ..." required>
						<div class="input-group-addon" id="icon-serial" style="font-weight: bold; font-size: 2vw; border-color: black;">
							<i class="glyphicon glyphicon-credit-card"></i>
						</div>
					</div>
...
<script>
	...
	$('#tag_product').keyup(function(event) {
		if (event.keyCode == 13 || event.keyCode == 9) {
			if($("#tag_product").val().length >= 7){
				...
				$.get('{{ url("scan/recorder/tag_product") }}', data, function(result, status, xhr){
					if(result.status){
						openSuccessGritter('Success!', 'Scan Tag Success');
						$('#tag_product').prop('disabled',true);
						$.each(result.data, function(key, value) {
							fetchReturn(value.material_number, value.material_description,value.issue_location,value.receive_location);
						})
					}
					else{
	...
	function printReturn(){
		...
		if(material == ''){
			$('#loading').hide();
			openErrorGritter('Error!', 'Scan material yang akan di return');
			return false;
		}
		...
		$.post('{{ url("index/push_block_recorder/return_completion") }}', data2, function(result, status, xhr){
			if(result.status){
				openSuccessGritter('Success', result.message);
			}
			else{
