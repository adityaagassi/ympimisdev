      ...
      <div class="col-md-12" style="padding-right: 0;padding-left: 10px">
          <table id="tabelmonitor" class="table table-bordered" style="margin-top: 5px; width: 99%">
            <thead style="background-color: rgb(255,255,255); color: rgb(0,0,0); font-size: 12px;font-weight: bold">
              <tr>
                <th style="width: 10%; padding: 0;vertical-align: middle;;font-size: 16px;">Audit Kategori</th>
                <th style="width: 10%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;">Tanggal</th>
                <th style="width: 15%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;">Lokasi</th>
                <th style="width: 15%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;">CPAR</th>
                <th style="width: 15%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;">Standarisasi</th>
                <th style="width: 15%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;">Penanganan</th>
                <th style="width: 15%; padding: 0;vertical-align: middle;border-left:3px solid #f44336 !important;font-size: 16px;background-color:#448aff;">Verification</th>
              </tr>
            </thead>
            <tbody id="tabelisi">
            </tbody>
...
<script>
    ...
    $.get('{{ url("fetch/audit_iso/monitoring") }}', data, function(result, status, xhr) {
        if(result.status){
          ...
          $.each(result.datas, function(key, value) {
            ...
            reject.push(parseInt(value.rejected));
          ...
          $('#chart').highcharts({
            ...
            title: {
              text: 'Monitoring Form Audit ISO E-IRCA',
              style: {
                fontSize: '25px',
                fontWeight: 'bold'
              }
  ...
  function ShowModal(tgl, status, datefrom, dateto) {
    ...
    $('#judul_table').append('<center><b>List Audit ISO Tanggal '+tgl+'</b></center>'); 
  }
    ...
    $.get('{{ url("index/audit_iso/table") }}', data, function(result, status, xhr){
      if(result.status){
          ...
          $.each(result.datas, function(key, value) {
            ...
            table += '<td style="border-left:3px solid #f44336"><span class="label label-warning">'+value.auditor_date+'</span></td>';
            table += '<td style="border-left:3px solid #f44336">'+value.auditor_lokasi+'</td>';
            table += '<td style="border-left:3px solid #f44336">'+statusauditor+'</td>';
            table += '<td style="border-left:3px solid #f44336">'+statusstd+'</td>';
            table += '<td style="border-left:3px solid #f44336">'+statusauditee+'</td>';
            table += '<td style="border-left:3px solid #f44336">'+statusverif+'</td>';
            table += '</tr>';
          })
