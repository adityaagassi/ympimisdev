  ...
  <div class="col-xs-4">
    <div class="info-box"> 
      <div class="info-box-content" style="margin:0px">
        ...
        <div class="table-responsive">
          <table width="100%"  class="table  table-striped" border="0" style="margin :0px" id="opFokiesTable">
          </table>  
        </div>
      </div>
    </div>
  ...
  $.get('{{ url("fetch/opTunning") }}', data, function(result, status, xhr){
    if(xhr.status == 200){
      if(result.status){
        ...
        console.table(result.opTunning);
        $('#planTableBody').html("");
        ...
        var planData2 = '';
        planData2 += '<tr>';
        $.each(result.opFokies, function(key, value) {
          planData2 += '<td> <button class="btn btn-md btn-danger" style="font-size:30px; width:90%" id="'+value.nik+'"'+'name="'+value.name+'" onclick="opkar(this.id,this.name,this)">'+ value.kode +'</td>';
        }); 
        planData2 += '</tr>';  
        $('#opFokiesTable').append(planData2); 
