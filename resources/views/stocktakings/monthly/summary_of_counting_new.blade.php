					...
					<div class="row">
						<div class="col-xs-3 col-xs-offset-3">
							<div class="form-group">
								<label>Group</label>
								<select class="form-control select2" multiple="multiple" name="filter_area" id='filter_area' data-placeholder="Select Group" style="width: 100%;">
									<option value=""></option>
					...
					<div class="row">
						...
						<div class="col-xs-3">
							<div class="form-group">
								...
								<select class="form-control select2" multiple="multiple" name="filter_substore" id='filter_substore' data-placeholder="Select Substore (substore - store)" style="width: 100%;">
									<option value=""></option>
									@foreach($substores as $substore) 
									<option value="{{ $substore->sub_store }} - {{ $substore->store }}">{{$substore->sub_store}} - {{ $substore->store }}</option>
									@endforeach
								</select>
							</div>
						</div>		
...
<script>
	...
	function printJob(element){
		...
		// $("#loading").show();
		// console.log('{{ url("reprint/stocktaking/summary_of_counting_id_new/") }}/'+tag.join(","));

		window.open('{{ url("reprint/stocktaking/summary_of_counting_id_new/") }}/'+tag.join(","), '_blank');

		// $.get('{{ url("reprint/stocktaking/summary_of_counting_id_new") }}', data, function(result, status, xhr){
		// 	if(result.status){
		// 		$("#loading").hide();
		// 		openSuccessGritter('Success', result.message);
		...
		$.get('{{ url("fetch/stocktaking/new_store_details") }}', data, function(result, status, xhr){
			if (result.status) {
				...
				for (var i = 0; i < result.data.length; i++) {
					...
					body += '<td onClick="countPicked(this)" id="td3+'+result.data[i].id+'">'+result.data[i].sub_store+'</td>';
