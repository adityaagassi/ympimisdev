											...
											<div class="col-md-4">
												<div class="form-group">
													...
													<select class="form-control select2" data-placeholder="Pilih Approver" name="approvedBy" id="approvedBy" style="width: 100% height: 35px; font-size: 15px;">
														<option value=""></option>
														<option value="PI9707002">Anang Sutikno</option>
														<option value="PI9910003">Mahdhi Ichromi</option>
														<option value="PI9906003">Bambang Supriyadi</option>
													</select>
												</div>
											</div>
										</div>
...
<script>
		...
		$.get('{{ url("fetch/maintenance/list_spk") }}', data, function(result, status, xhr){
			if(result.status){
				...
				$.each(result.tableData ,function(index, value){
					...
					tableData += "<td>"+(value.approval || '-')+"</td>";

					value.operator_id != null ? pic = value.operator_id.replace(',','<br>') : pic = '-'
	...
	function pilih() {
		...
		body += "<tr id='"+employee_id+"'>";
		body += "<td>"+counter+"</td>";
		body += "<td id='operator_"+counter+"'>"+employee_id+"</td>";
		body += "<td>"+employee_name+"</td>";

		body += "<td>";
		body += "<div class='input-group'>";
		body += "<input type='text' class='form-control datepicker' id='start_date_"+counter+"' placeholder='Start Date' style='width:60%'>";
		body += "<input type='text' class='form-control timepicker' id='start_time_"+counter+"' placeholder='Start Time' style='width:40%'>";
		body += "</div>";
		body += "</td>";

		body += "<td>";
		body += "<div class='input-group'>";
		body += "<input type='text' class='form-control datepicker' id='finish_date_"+counter+"' placeholder='Finish Date' style='width:60%'>";
		body += "<input type='text' class='form-control timepicker' id='finish_time_"+counter+"' placeholder='Finish Time' style='width:40%'>";
		body += "</div>";
		body += "</td>";
	...
	function job_ok() {
		...
		for (var i=1; i < counter; i++) { 
			var employee_id = $("#operator_"+i).text();

			var start_date = $("#start_date_"+i).val();

			var start_time = $("#start_time_"+i).val();
			var start_time = format_two_digits(start_time.split(':')[0])+":"+start_time.split(':')[1];
			var finish_date = $("#finish_date_"+i).val();

			var finish_time = $("#finish_time_"+i).val();
			var finish_time = format_two_digits(finish_time.split(':')[0])+":"+finish_time.split(':')[1];
