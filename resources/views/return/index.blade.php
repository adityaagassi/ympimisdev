					...
					<table class="table table-hover table-striped" id="tableList">
						<thead>
							<tr>
								...
								<th style="width: 1%;">Issue Loc.</th>
								<th style="width: 1%;">Receive Loc.</th>
							</tr>					
						</thead>
		...
		<div class="col-xs-7">
			<div class="row">
				...
				<div class="col-xs-12">
					<div class="box">
						<div class="box-body">
							<span style="font-size: 20px; font-weight: bold;">RETURN BELUM DI KONFIRMASI ({{ date('d-M-Y') }})</span>
...
<script>
	...
	function receiveReturn(video, data){
		...
		$.get('{{ url("fetch/return") }}', x, function(result, status, xhr){
			if(result.status){
				var re = "";
				$('#receiveReturn').html("");
	...
	function confirmReceive(id){
		...
		$.post('{{ url("confirm/return") }}', data, function(result, status, xhr){
			if(result.status){
	...
	function printReturn(){
		...
		$.post('{{ url("print/return") }}', data, function(result, status, xhr){
			if(result.status){
				fetchResume(receive);
				$('#material_number').val("");
				$('#issue').val("");
	...
	function reprint(id){
		...
		$.get('{{ url("reprint/return") }}', data, function(result, status, xhr){
			if(result.status){
				openSuccessGritter('Success!', result.message);
			}
			else{
	...
	function fetchResume(loc){
		...
		$.get('{{ url("fetch/return/resume") }}', data, function(result, status, xhr){
	...
	function fetchReturnList(loc){
		...
		$.get('{{ url("fetch/return/list") }}', data, function(result, status, xhr){
			if(result.status){
