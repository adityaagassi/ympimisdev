...
public function scanKdCardCleaning(Request $request)
{
	...
	$inventory = AssemblyInventory::where('serial_number',$details->serial_number)->where('origin_group_code',$details->origin_group_code)->first();
...
public function fetchSerialNumberReport($process,Request $request)
{
	...
	COALESCE ((
		SELECT
			GROUP_CONCAT( CONCAT( employee_syncs.employee_id, '<br>', employee_syncs.NAME ) ) 
		FROM
			assembly_logs
			LEFT JOIN employee_syncs ON employee_syncs.employee_id = assembly_logs.operator_id 
		WHERE
