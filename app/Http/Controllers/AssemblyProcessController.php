	...
	public function scanAssemblyKensa(Request $request)
	{
		...
		public function inputAssemblyKensa(Request $request)
		{
			if($request->get('tag')){
				...
				$ng_temp = AssemblyNgTemp::where('model',$model)->where('serial_number',$serial_number)->where('employee_id',$employee_id)->where('tag',$tag)->where('origin_group_code','041')->get();
				...
				$assembly_invent = AssemblyInventory::where('model',$model)->where('serial_number',$serial_number)->where('tag',$tag)->where('origin_group_code','041')->first();

				$remark = $assembly_invent->remark;

				if ($remark == 'SP') {
					$assembly_details = AssemblyDetail::where('model',$model)->where('serial_number',$serial_number)->where('tag',$tag)->where('origin_group_code','041')->delete();
				}else{
				...
				try{
					...
					$assembly_inventories = AssemblyInventory::where('model',$model)->where('serial_number',$serial_number)->where('tag',$tag)->where('origin_group_code','041')->first();
