...
->addColumn('action',function($pr){
  $id = $pr->id;
  return '
  <a href="javascript:void(0)" data-toggle="modal" class="btn btn-xs btn-warning" class="btn btn-primary btn-sm" onClick="editPR('.$id.')"><i class="fa fa-edit"></i></a>
  <a href="purchase_requisition/detail/'.$id.'" class="btn btn-info btn-xs"><i class="fa fa-eye"></i></a>
  ';
})
...
{
  $items = AccItem::select('acc_items.kode_item','acc_items.deskripsi')->limit(1000)->get();
  ...
  $mailtoo = DB::select($mails);
    ...
    //==================================//
        ...
        public function edit_purchase_requisition(Request $request)
        {
          ...
          $purchase_requistion_item = AccPurchaseRequisition::join('acc_purchase_requisition_items','acc_purchase_requisitions.no_pr','=','acc_purchase_requisition_items.no_pr')->where('acc_purchase_requisitions.id', '=', $request->get('id'))->get();
    ...
    //==================================//
        ...
        public function fetch_purchase_order(Request $request)
        {
         ...
         ->addColumn('action',function($po){
          ...
          return '<a href="javascript:void(0)" data-toggle="modal" class="btn btn-xs btn-warning" class="btn btn-primary btn-sm" onClick="editPO('.$id.')"><i class="fa fa-edit"></i></a>';
        })
      ...
      {
        $pr = AccPurchaseRequisition::select('acc_purchase_requisitions.no_pr')
      ...
      {
        $html = array();
        $list_item = AccPurchaseRequisitionItem::where('no_pr',$request->no_pr)->whereNull('sudah_po')->get();
      ...
      {
        $html = array();
        $kode_item = AccPurchaseRequisitionItem::join('acc_purchase_requisitions','acc_purchase_requisition_items.no_pr','=','acc_purchase_requisitions.no_pr')->where('item_code',$request->item_code)->where('acc_purchase_requisition_items.no_pr',$request->no_pr)->get();
        foreach ($kode_item as $item) {
          $html = array(
      ...
      public function create_purchase_order(Request $request)
      {
        ...
        try{
          ...
          for ($i=1; $i <= $lop ; $i++) {
            ...
            $data3 = AccPurchaseRequisitionItem::where('item_code',$request->get($no_item))->where('no_pr',$request->get($no_pr))
            ->update([
             'sudah_po' => 'true',
           ]);
          }

          return redirect('/purchase_order')->with('status', 'Purchase Order Berhasil Dibuat')->with('page', 'Purchase Order');
        }
        catch (QueryException $e){
          return redirect('/purchase_order')->with('error', $e->getMessage())->with('page', 'Purchase Order');
        }
      }
      ...
      public function edit_purchase_order(Request $request)
      {
        ...
        $purchase_order_detail = AccPurchaseOrder::join('acc_purchase_order_details','acc_purchase_orders.no_po','=','acc_purchase_order_details.no_po')->where('acc_purchase_orders.id', '=', $request->get('id'))->get();
      ...
      public function update_purchase_order(Request $request)
      {
        ...
        try{
          ...
          ->update([
            'supplier' => $request->get('supplier_edit'),
          ...
          foreach ($lop as $lp) {
            ...
            $data2 = AccPurchaseOrderDetail::where('id',$lp)
              ->update([
                'no_item' => $request->get($no_item),
                'nama_item' => $request->get($nama_item),
                'budget_item' => $request->get($item_budget),
          ...
          return redirect('/purchase_order')->with('status', 'Purchase Order Berhasil Dirubah')->with('page', 'Purchase Order');
        }
        catch (QueryException $e){
          return redirect('/purchase_order')->with('error', $e->getMessage())->with('page', 'Purchase Order');
        }
      ...
      public function delete_item_po(Request $request)
      {
       try{
        ...
        $data3 = AccPurchaseRequisitionItem::where('item_code',$item->no_item)->where('no_pr',$item->no_pr)
        ->update([
          'sudah_po' => null,
        ]);
