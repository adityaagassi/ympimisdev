...
class AssyProcessController extends Controller
{
	...
	public function fetchPicking(Request $request, $id){
		...
		(
		select materials.material_number, 0 as plan, sum(if(histories.transfer_movement_type = '9I3', histories.lot, if(histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 2, 0, -(histories.lot)),0))) as picking, 0 as plus, 0 as minus, 0 as stock, 0 as plan_ori from
		(
		...
		(
		select materials.material_number, -(sum(if(histories.transfer_movement_type = '9I3', histories.lot, if(histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 2, 0, -(histories.lot)),0)))) as plan, 0 as plan_ori from
		(
		select materials.id, materials.material_number from kitto.materials where materials.location = '".$location."' and category = 'key'
		...
		select materials.material_number, 0 as plan, 0 as picking, sum(if(histories.transfer_movement_type = '9I3', histories.lot,0)) as plus, sum( if(histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 2, 0, histories.lot),0)) as minus, 0 as stock, 0 as plan_ori from
		(
		select materials.id, materials.material_number from kitto.materials where materials.location = '".$location."' and category = 'key'
