...
class SkillMapController extends Controller
{
    ...
    {
    	$employee_skill = SkillEmployee::orderBy('process','desc')->get();
    	$employees = EmployeeSync::orderBy('name', 'asc')->get();
    	$process = DB::SELECT("SELECT DISTINCT(process) from skill_employees order by process");
    ...
    public function fetchSkillMap(Request $request)
    {
    	try {
    		...
    		$process = DB::SELECT("SELECT DISTINCT(process) FROM `skills` where location = '".$location."' order by process ");
    ...
    public function fetchSkillEmployee(Request $request)
    {
    	try {
    		...
    		$process = Skill::select('process')->distinct()->where('location',$request->get('location'))->get();
    		$employees = EmployeeSync::where('department','like','%'.$request->get('dept').'%')->where('section','like','%'.$request->get('section').'%')->get();
