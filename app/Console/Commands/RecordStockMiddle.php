    ...
    public function handle()
    {
        ...
        try{
            $stockroom_keys = db::select("INSERT INTO middle_stocks (
                ...
                histories.lot,
                IF
                ( histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 3, 0, -(histories.lot)),0))) AS picking,
                0 AS plus,
                0 AS minus,
                0 AS stock,
                0 AS plan_ori 
                ...
                histories.lot,
                IF
                ( histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 3, 0, -(histories.lot)),0)))) AS plan,
                0 AS plan_ori 
                FROM
                (
                SELECT
                ...
                sum(
                IF
                ( histories.transfer_movement_type = '9I4', IF(day(histories.created_at) < 3, 0, histories.lot),0)) AS minus,
                0 AS stock,
                0 AS plan_ori 
                FROM
                (
