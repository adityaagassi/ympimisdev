...
class SendEmailShipments extends Command
{
    ...
    {
        $mail_to = db::table('send_emails')->where('remark', '=', 'shipment')->select('email')->get();
        $st_date = date('Y-m-d', strtotime(Carbon::now()->addDays(2)));

        $query = "
        select a.hpl, a.st_date, a.material_number, a.material_description, a.destination_shortname, a.plan, coalesce(b.actual,0) as actual, coalesce(b.actual,0)-a.plan as diff from
        (
